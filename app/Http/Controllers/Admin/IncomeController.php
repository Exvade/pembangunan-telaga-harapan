...
class IncomeController extends Controller
{
    public function index(\Illuminate\Http\Request $r)
    {
        ...
        $items = \App\Models\Income::when($from, fn($q) => $q->whereDate('date', '>=', $from))
            ->when($to, fn($q) => $q->whereDate('date', '<=', $to))
            ->latest('date')
            ->paginate(12)
            ->withQueryString();

        return view('admin.incomes.index', compact('items', 'from', 'to'));
    }
    ...
    {
        $data = $r->validate([
            // 'category_id' => 'nullable|exists:categories,id', // kalau tidak ingin dipakai sama sekali, hapus baris ini
