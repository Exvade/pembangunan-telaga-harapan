...
@section('content')
    ...
    <div x-data="{ modalOpen: false, modalContentUrl: '', isModalContentImage: false }"
        ...
        <section class="bg-slate-800 text-white">
            <div class="container mx-auto px-4 py-12">
                <div class="max-w-4xl mx-auto">
                    <a href="{{ route('public.transparency') }}" class="text-sm font-semibold text-slate-300 hover:text-white transition-colors inline-flex items-center gap-2">
                    ...
                    @if($category->description)<p class="text-slate-300 mt-2 max-w-2xl">{{ $category->description }}</p>@endif
                </div>
            </div>
        </section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                        ...
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <div class="text-2xl font-bold text-slate-800 mt-1">Rp {{ number_format($totalExpense,0,',','.') }}</div>
                            </div>
                        </div>
                        {{-- Status --}}
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-start gap-4">
                             <div class="rounded-lg p-3 {{ $category->is_active ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-600' }}">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            </div>
                            <div>
                                <div class="text-sm font-medium text-slate-500">Status</div>
                                <div class="text-2xl font-bold mt-1 flex items-center gap-2">
                                     <span class="px-2.5 py-1 rounded-full text-sm font-medium {{ $category->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700' }}">
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200">
                            <ul class="divide-y divide-slate-100">
                                @forelse($expenses as $e)
                                     @php
                                        $isAttachmentImage = $e->attachment_mime_type ? \Illuminate\Support\Str::contains($e->attachment_mime_type, 'image') : false;
                                    @endphp
                                    <li class="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                        <div>
                                            <p class="text-xs text-slate-500">{{ \Illuminate\Support\Carbon::parse($e->date)->format('d F Y') }}</p>
                                        ...
                                        <div class="flex items-center gap-4 flex-shrink-0">
                                            <p class="font-semibold text-rose-600">Rp {{ number_format($e->amount,0,',','.') }}</p>
            ...
            <template x-if="!isModalContentImage && modalOpen">
                <div @click.stop class="relative w-full max-w-4xl h-[90vh] bg-white rounded-lg shadow-xl overflow-hidden">
                    <iframe :src="modalContentUrl" class="w-full h-full" frameborder="0"></iframe>
                     <button @click="modalOpen = false" class="absolute top-2 right-2 z-20 p-2 rounded-full bg-black/20 text-white hover:bg-black/50">
