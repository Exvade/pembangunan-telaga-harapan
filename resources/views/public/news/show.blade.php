                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach ($item->media as $m)
                                    @if ($m->type === 'image' && $m->file_path)
                                        <figure class="bg-white border border-slate-200 rounded-xl overflow-hidden">
                                            <a href="{{ Storage::url($m->file_path) }}" target="_blank"
                                                title="Lihat gambar">
                                                <img loading="lazy" decoding="async" src="{{ Storage::url($m->file_path) }}"
                                                    alt="{{ $m->caption ?? 'Foto kegiatan' }}"
                                                    class="w-full h-48 object-cover">
                                            </a>
                                            @if ($m->caption || $m->credit)
                                                <figcaption class="p-2 text-xs text-slate-600">
                                                    {{ $m->caption }}
                                                    @if ($m->credit)
                                                        <span class="text-slate-400"> — {{ $m->credit }}</span>
                                                    @endif
                                                </figcaption>
                                            @endif
                                        </figure>
                                    @elseif($m->type === 'video' && $m->embed_url)
                                        <div class="bg-black rounded-xl overflow-hidden">
                                            <div class="aspect-video">
                                                <iframe src="{{ embed_url($m->embed_url) }}" class="w-full h-full"
                                                    frameborder="0"
                                            ...
                                            @if ($m->caption)
                                                <div class="p-2 text-xs text-slate-300">{{ $m->caption }}</div>
                                            @endif
                                        </div>
                                    @elseif($m->type === 'video' && $m->file_path)
                                        <div class="bg-black rounded-xl overflow-hidden">
                                            <video class="w-full" controls preload="none">
                                                <source src="{{ Storage::url($m->file_path) }}"
                                    ...
                                    @elseif($m->type === 'file' && $m->file_path)
                                        <div
                                            class="bg-white border border-slate-200 rounded-xl p-3 flex items-center justify-between">
                                            <div class="text-sm">
                                                <div class="font-medium">Dokumen</div>
                                                <div class="text-xs text-slate-500">{{ $m->caption ?? 'Lampiran' }}</div>
                                            </div>
                                            <a class="text-blue-600 underline text-sm" target="_blank"
                                                href="{{ Storage::url($m->file_path) }}">Lihat</a>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div x-data="{ copied: false }" class="relative">
                                <button
                                    @click="navigator.clipboard.writeText('{{ route('public.news.show', $item->slug) }}'); copied = true; setTimeout(() => copied = false, 2000)"
