...
@section('content')
    <div class="bg-slate-50 min-h-screen p-4 sm:p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-start gap-4">
                ...
                <div>
                    ...
                    <div class="text-2xl sm:text-3xl font-bold text-slate-800 mt-1">Rp {{ number_format($totalIn, 0, ',', '.') }}</div>
                </div>
            </div>
            {{-- Total Pengeluaran --}}
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-start gap-4">
                ...
                <div>
                    ...
                    <div class="text-2xl sm:text-3xl font-bold text-slate-800 mt-1">Rp {{ number_format($totalEx, 0, ',', '.') }}</div>
                </div>
            </div>
            {{-- Saldo --}}
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-start gap-4">
                ...
                <div>
                    ...
                    <div class="text-2xl sm:text-3xl font-bold mt-1 {{ $saldo < 0 ? 'text-rose-600' : 'text-slate-800' }}">
            ...
            {{-- Berita Terpublikasi --}}
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-start gap-4">
                 <div class="bg-sky-100 text-sky-600 rounded-lg p-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg>
                </div>
                <div>
                    <div class="text-sm font-medium text-slate-500">Berita Terpublikasi</div>
                    <div class="text-2xl sm:text-3xl font-bold text-slate-800 mt-1">{{ $newsPublished }}</div>
                </div>
            </div>
        </div>
        ...
        {{-- === Bulan Berjalan === --}}
        <div class="mt-8">
             <h2 class="text-lg font-semibold text-slate-700 mb-3">Ringkasan Bulan Ini ({{ $monthText }})</h2>
             <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <div class="text-sm font-medium text-slate-500">Pemasukan</div>
                    <div class="text-2xl font-bold text-emerald-600 mt-1">Rp {{ number_format($monthIn, 0, ',', '.') }}</div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <div class="text-sm font-medium text-slate-500">Pengeluaran</div>
                    <div class="text-2xl font-bold text-rose-600 mt-1">Rp {{ number_format($monthEx, 0, ',', '.') }}</div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <div class="text-sm font-medium text-slate-500">Saldo</div>
                    <div class="text-2xl font-bold mt-1 {{ $monthSaldo < 0 ? 'text-rose-600' : 'text-slate-800' }}">
                        Rp {{ number_format($monthSaldo, 0, ',', '.') }}
                    </div>
                </div>
            </div>
        </div>

        {{-- === Dua kolom: Top Pengeluaran & Transaksi Terbaru === --}}
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">

            {{-- KIRI: Pengeluaran Terbesar --}}
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
